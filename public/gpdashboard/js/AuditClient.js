...
//To create a new function in registry must create the get{item}Result where
//function name is {item}
//It is important that the get{item}Result function return result object with
//parameters referenced in the registry template
...
Audit.prototype.validate = function(doc, fieldsToValidate) {
  ...
  //Not the same as clearing which empties everything out and sets compliance to
  //false
  ...
  //If they do not have all update fields in Audit errors then just add empty
  //object now
...
//out errors. Passing fields only clears out those fields but keeps errors
//intact for previously created errors. This is important for persisting errors
//and only producing incremental change in objects errors
...
Object.keys(Audit.prototype.registry).forEach(function(key) {
  ...
  var checkFunctionName = 'check' + key.charAt(0).toUpperCase() +
      key.substring(1);
  var getResultsFunctionName = 'get' + key.charAt(0).toUpperCase() +
      key.substring(1) + 'Result';
  //If not processItems variable set then defaults to true. must set to false to
  //get false ie. falsey empty value will default to true.
  ...
  Audit.prototype[checkFunctionName] = function() {
    ...
    args = Array.prototype.concat(
        [this[getResultsFunctionName], processItems], args);
...
//eg:  this.checkForbiddenWords("title",doc,[" test ","-copy "]) calls
//getRequiredFieldResult(doc.title,[" test ","-copy "]) ;
//It is also possible to pass array to check{Validation} as first argument which
//will validate all fields. There is no need to change get{Validation}Result to
//accomodate this and it should be written to handle individual doc.fields
...
//getForbiddenWords returns
//{pass:True/False,matches:[forbidden words that were matched]}
//get{Validation}Result can also return an array of results if more than one
//result/error was returned
//It is important to make the function name of get{Validation}Result to be
//{Validation} for eaiser identification. I am thinking about changing this
//requiremnt
...
//This basically does the same thing as getForbiddenWordsResult but allows a
//custom error to be passed. It had to be a new function because we had to make
//a new template equal to <<error>> instead of the standard
Audit.prototype.getRegexMatchResult = function regexMatch(myString,
                                                          forbidden,
                                                          error) {
...
Audit.prototype.getWordLimitResult = function wordLimit(myString,
                                                        floor,
                                                        ceiling) {
  var wordCount = 0;
  if (myString) {
...
Audit.prototype.getArrayLimitResult = function arrayLimit(myArray,
                                                          floor,
                                                          ceiling) {
...
Audit.prototype.checkGeneric = function(myFunction, processItems, fields, doc) {
  ...
  fields.forEach(function(field) {
    ...
    if (self.fieldsToValidate.length === 0 ||
      ...
      var checkSingleArgs = Array.prototype.concat(
          [myFunction, processItems, field], args);
...
Audit.prototype.checkGenericSingle = function(myFunction,
                                              processItems,
                                              field,
                                              doc) {
  ...
  //This matches forbidden words and creates error messags and attaches to audit
  //doc.AuditData
  ...
  values.forEach(function(value, index) {
    ...
    //Allow multiple results from one call. Useful for testing multiple items
    //in array
