...
//          if (callback) callback(i);
      ...
      for (i == 0; i < data.length; i++) {
        createInterval(function (k) {
          self.content.push(new egam.gpoItemModel(data[k], true));
          if (callback) callback(k);
          if (k == data.length - 1) {
            console.log("resolve k" + k);
            defer.resolve();
          }
        }, i, 0);
//          self.content.push(new egam.gpoItemModel(data[i], true));
...
//      self.content = [];
      for (i == 0; i < data.length; i++) {
//          self.content.push(new egam.gpoItemModel(data[i], false));
