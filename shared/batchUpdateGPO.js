module.exports = function(updateDocs, getUpdateClassInstance, updateName, updateID, useSync, AsyncRowLimit) {
  ...
  //Note:  use fcall so that if exception in updateGPOfunction still returns promise
    .catch(function(err) {
    ...
    .then(function() {
      ...
      if (resObjects.errors.filter(function(x) {
          return x && x.length > 0
        }).length == 0) resObjects.errors = null;
      return resObjects
    });
  ...
  function updateSingleGPO(updateDoc, updateRow, async) {
    ...
    if (!updateRow) updateRow = updateGPOrow;
    ...
    return updateGPOinstance.update(updateDoc)
      ...
      .catch(function(err) {
        resObjects.errors.push('Error updating ' + updateDoc[updateID] + ' : ' + err.message);
