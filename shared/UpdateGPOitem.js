  ...
  return self.getOwnerID()
    .then(function(ownerID) {
      if (self.updateOwner) {
        ...
        self.resObject = {
          error: {
            message: "You do not have Access to Update GPO item: " + self.updateDoc.id,
            code: "InvalidAccess"
          },
          body: null
        };
    ...
    .then(function() {
      if (self.thumbnail && self.thumbnail.path) return Q.ninvoke(fs, "unlink", self.thumbnail.path)
    })
    .catch(function(err) {
...
UpdateGPOitem.prototype.getOwnerID = function() {
  ...
  return Q(self.itemscollection.findOne({
      id: GPOid
    }, {
      fields: {
        owner: 1,
...
UpdateGPOitem.prototype.updateRemoteGPOitem = function() {
  ...
  var utilities = require(appRoot + '/shared/utilities');
  var formData = utilities.sliceObject(self.updateDoc, ["title", "description"]);

  //get read stream from uploaded thumb file and add to form data with name
  if (self.thumbnail) formData.thumbnail = {
    value: fs.createReadStream(self.thumbnail.path),
    options: {
...
UpdateGPOitem.prototype.updateLocalGPOitem = function() {
  ...
  return Q.all([
    Q(this.itemscollection.update({
      id: this.updateDoc.id
    }, {
      $set: this.updateDoc
    })),
...
UpdateGPOitem.prototype.saveThumbnailToGridFS = function() {
  ...
  writestream.on('close', function(file) {
    ...
    Q(self.itemscollection.update({
      id: self.updateDoc.id
    }, {
      $set: {
        SlashData: self.updateSlashData
      }
    }))
      .catch(function(err) {
