...
TasksQueues.prototype.getProcess = function() {
  ...
  return function process(jinn, data) {
    //Start this Q.fcall because data.promise(data.arg) might not return promise
    //if there is exception thrown
    Q.fcall(data.promise,data.arg)
      //If there is an error in data.promise function then need to reject the
    //defer passed to promise if there is one
      .catch(function(err) {if (data.arg.defer) {
        data.arg.defer.reject(err);
      }})
      //This is old way of doing it that would create unhandled exception.
      //Uncomment this if want to test server restart recovering from
      //unhandled excpetion crash
      ...
      function() {
        ...
        //Uncomment this if we think having too many queues for each user is
        //using too much memory
        if (length < 1) {
          delete self.queues[data.arg.task];
        }
      });
  };
};
