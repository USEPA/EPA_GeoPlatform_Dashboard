module.exports = function(app) {
  ...
  router.use('/list', function(req, res) {
    ...
    inclusionFields = Object.keys(projection.fields)
    ...
    function getTotalRowsPromise() {
      ...
      //count when paging
      if ('session' in req && 'GPOitemsList' in req.session &&
          req.session.GPOitemsList.query == JSON.stringify(query)) {
      ...
      return Q(itemscollection.count(query))
        .then(function(total) {
          if ('session' in req) {
            req.session.GPOitemsList = {
              query: JSON.stringify(query),
              total: total,};
          }
          console.log('Save Session: ' +
            JSON.stringify(req.session.GPOitemsList));
          return total;
        });
    }
