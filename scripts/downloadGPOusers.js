...
//Set this to false for the overnight download so that we don't keep the
//removed items locally
...
//Just a test to see if this works. This would usually be called in Log in if
//it is decided that Mod Users need to be refreshed on log in
...
try {
  ...
  if (!downloadGPOusers.appID || !downloadGPOusers.appSecret) {
    throw '';
  }
}
catch (e) {
  downloadGPOusers.downloadLogs.log(
...
try {
  ...
  if (!downloadGPOusers.mongoDBurl) {
    throw '';
  }
} catch (e) {
...
try {
  if ('AGOLorgID' in config) {
    downloadGPOusers.orgID = config.AGOLorgID;
  }
} catch (e) {
}
...
if (downloadGPOusers.downloadLogs.errors.length == 0) {
  downloadGPOusers.download()
    .catch(function(err) {
      ...
      downloadGPOusers.downloadLogs.error(
        'Error while downloading GPO user data, group data and owner IDs using downloadGPOusers.download():\n' + errMsg);
    })
    .done(function() {
