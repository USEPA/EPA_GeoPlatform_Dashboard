...
//temporary slice for testing
    if (TotalRowLimit && count > TotalRowLimit) return;
...
//    var token = getHandleResponsePromise('token')(data);
  ...
  .then(function (user) {
      if (! sponsorExists) return;
      if (! user) {
        console.log("External Username " + currentItem.username+ " doesn't exist");
        return;
      }
      if (! user.isExternal) {
        console.log("External Username " + currentItem.username+ " with provider = " + user.provider + " and email = " + user.email + "is not an External User");
        return;
      }
...
function updateExternalUser(item) {
  ...
  //note: need to get "mock" session info because update checks permission to update user (needs to be admin to update external users)
  var session = {ownerIDs:[],user:{isAdmin:true},token:hr.saved.token};

  var updateInstance = new UpdateGPOclass(usersCollection,extensionsCollection,session,config);
...
//This will acutally update the authGroup on GPO (and locally on user not just sponsor)
...
function addSponsorsHybrid() {
  return hr.promiseWhile(function() {return currentRow<=totalRows;}, addSponsorsAsync);
}
...
//take slice form current row to async row limit
    externalUsersSlice= externalUsers.slice(currentRow-1,currentRow-1+AsyncRowLimit);
  }else {
    externalUsersSlice= externalUsers;
  }

  //need to get the value of currentRow when this function is called because addSingleSponsor changes it
  //THis is basically the currentRow when the async loop started
  var asyncStartcurrentRow = currentRow;

  console.log('currentRow and externalUsers.length ' + currentRow + ' ' + externalUsers.length);
...
//          done();})
        .catch(function(err) {
          console.error('single sponsor add Error received at ' + externalUsers[key+asyncStartcurrentRow-1].username + ':');
