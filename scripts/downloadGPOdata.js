...
try {
  ...
  //Make sure downloadGPOdataConfig is an object
  if (typeof downloadGPOdataConfig !== 'object') {
    downloadGPOdataConfig = {};
  }
}catch (e) {
}
...
//How many aysnc slash data rows can be downloaded at one time. too many and
//things crash
//set to null to run all at one time (can crash for large data sets)
//setting to 0 or 1 is the same as useSync=true
...
//Set this to false for the overnight download so that we don't keep the removed
//items locally
//But don't do this if you are downloading the entire DB at first because
//querying on AGOL by Modified Date is actually slow over entire data set
...
if (downloadGPOdata.HardCodeSlashDataTestIDs) {
  downloadGPOdata.HardCodeSlashDataTestIDs
    .push('091f277d34aa4e4996d142b2585bd12c');
}
...
try {
  ...
  if (!downloadGPOdata.appID || !downloadGPOdata.appSecret) {
    throw '';
  }
}
catch (e) {
  downloadGPOdata.downloadLogs.log(
    'AGOL app ID or secret not defined in config file, trying usename/password');
  try {
    ...
    downloadGPOdata.downloadLogs.error(
      'AGOL admin username and password not defined in config file');
    downloadGPOdata.downloadLogs.emailExit();
  }
  if (!downloadGPOdata.username || !downloadGPOdata.password) {
    downloadGPOdata.downloadLogs.error(
      'AGOL admin username and password can not be empty');
    downloadGPOdata.downloadLogs.emailExit();
  }
}
  ...
  if (getTokenOnly) {
    downloadGPOdata.getToken()
      .catch(function(err) {
        ...
        downloadGPOdata.downloadLogs.error(
          'Error while getting Token using downloadGPOdata.getToken():\n' +
          errMsg);
      })
      .done(function() {
    ...
    downloadGPOdata.download()
      .catch(function(err) {
        var errMsg = err.stack || err;
        downloadGPOdata.downloadLogs.error(
          'Error while downloading GPO meta data and slash data using downloadGPOdata.download():\n' + errMsg);
      })
      .done(function() {
